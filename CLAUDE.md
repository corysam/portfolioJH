...
- `category` (collection): name, slug, `order` (integer, default 999 — ascending display rank; the lowest-ordered category leads the `Work` filter row and is the filter selected on load, i.e. the "main" one), `categoryColor` + `categoryTextColor` (both `plugin::color-picker.color` custom fields — the badge background/border and its label color, one of each per category), projects (manyToMany back-relation).
